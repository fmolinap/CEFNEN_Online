from PySide6.QtWidgets import (
    ...
    QComboBox, QTextEdit, QScrollBar, QFileDialog, QInputDialog, QApplication, QProgressBar
)
...
class RsyncThread(QThread):
    ...
    def run(self):
        process = subprocess.Popen(self.rsync_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
        while True:
...
class FetchDLTFiles(QWidget):
    ...
    def init_ui(self):
        ...
        fetch_button = QPushButton("Traer Archivos DLT")
        fetch_button.setStyleSheet("background-color: #4CAF50; color: white;")
        fetch_button.clicked.connect(self.fetch_dlt_files)
    ...
    def fetch_dlt_files(self):
        ...
        local_path = f"./dlt_files/{campaign}"
        os.makedirs(local_path, exist_ok=True)

        rsync_command = f"sshpass -p '{password}' rsync -avz -e 'ssh -p {port}' {user}@{ip}:{remote_path}/*.dlt {local_path}"

        self.progress_text.append("Iniciando transferencia de archivos DLT...\n")

        # Deshabilitar botones durante la transferencia
        self.findChild(QPushButton, "Traer Archivos DLT").setEnabled(False)
    ...
    def transfer_finished(self, success):
        ...
        self.findChild(QPushButton, "Traer Archivos DLT").setEnabled(True)
